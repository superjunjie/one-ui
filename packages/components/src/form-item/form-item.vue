<template>
  <Row :class="{ [baseClassName]: true }">
    ...
    <Row :class="wrapperClassName" v-if="props.name">
      ...
      <Col :span="wrapperCol">
        <span :class="{ [errorClassName]: true }" v-if="errorTip">{{
          errorTip
        }}</span>
      </Col>
    </Row>
...
import { FORM_CONTEXT_KEY } from '../form/common'
import type { FormContext, ValidateRule, ValidateResult } from '../form/common'
